...
class PaperweightDataConverters extends DataFixerBuilder implements com.sk89q.worldedit.world.DataFixer {
    ...
    private CompoundBinaryTag fixChunk(CompoundBinaryTag originalChunk, int srcVer) {
        net.minecraft.nbt.NBTTagCompound tag = (net.minecraft.nbt.NBTTagCompound) adapter.fromNativeBinary(originalChunk);
        net.minecraft.nbt.NBTTagCompound fixed = convert(LegacyType.CHUNK, tag, srcVer);
    ...
    private CompoundBinaryTag fixBlockEntity(CompoundBinaryTag origTileEnt, int srcVer) {
        net.minecraft.nbt.NBTTagCompound tag = (net.minecraft.nbt.NBTTagCompound) adapter.fromNativeBinary(origTileEnt);
        net.minecraft.nbt.NBTTagCompound fixed = convert(LegacyType.BLOCK_ENTITY, tag, srcVer);
        return (CompoundBinaryTag) adapter.toNativeBinary(fixed);
    }

    private CompoundBinaryTag fixEntity(CompoundBinaryTag origEnt, int srcVer) {
        net.minecraft.nbt.NBTTagCompound tag = (net.minecraft.nbt.NBTTagCompound) adapter.fromNativeBinary(origEnt);
        net.minecraft.nbt.NBTTagCompound fixed = convert(LegacyType.ENTITY, tag, srcVer);
        return (CompoundBinaryTag) adapter.toNativeBinary(fixed);
    }
    //FAWE end

    private String fixBlockState(String blockState, int srcVer) {
        net.minecraft.nbt.NBTTagCompound stateNBT = stateToNBT(blockState);
        Dynamic<net.minecraft.nbt.NBTBase> dynamic = new Dynamic<>(OPS_NBT, stateNBT);
        net.minecraft.nbt.NBTTagCompound fixed = (net.minecraft.nbt.NBTTagCompound) INSTANCE.fixer.update(DataConverterTypes.BLOCK_STATE, dynamic, srcVer, DATA_VERSION).getValue();
        return nbtToState(fixed);
    }

    private String nbtToState(net.minecraft.nbt.NBTTagCompound tagCompound) {
        StringBuilder sb = new StringBuilder();
        sb.append(tagCompound.getString("Name"));
        if (tagCompound.contains("Properties", 10)) {
            sb.append('[');
            net.minecraft.nbt.NBTTagCompound props = tagCompound.getCompound("Properties");
    ...
    private static net.minecraft.nbt.NBTTagCompound stateToNBT(String blockState) {
        ...
        if (propIdx < 0) {
            ...
            net.minecraft.nbt.NBTTagCompound propTag = new net.minecraft.nbt.NBTTagCompound();
    ...
    public DataFixer buildUnoptimized() {
        return this.fixer = new WrappedDataFixer(DataConverterRegistry.getDataFixer());
    }
    ...
    private class WrappedDataFixer implements DataFixer {
        ...
        public <T> Dynamic<T> update(TypeReference type, Dynamic<T> dynamic, int sourceVer, int targetVer) {
            ...
            if (sourceVer < LEGACY_VERSION && legacyType != null) {
                net.minecraft.nbt.NBTTagCompound cmp = (net.minecraft.nbt.NBTTagCompound) dynamic.getValue();
        ...
        private net.minecraft.nbt.NBTTagCompound convert(LegacyType type, net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int desiredVersion) {
    ...
    public static net.minecraft.nbt.NBTTagCompound convert(LegacyType type, net.minecraft.nbt.NBTTagCompound cmp) {
        return convert(type.getDFUType(), cmp);
    }

    public static net.minecraft.nbt.NBTTagCompound convert(LegacyType type, net.minecraft.nbt.NBTTagCompound cmp, int sourceVer) {
        return convert(type.getDFUType(), cmp, sourceVer);
    }

    public static net.minecraft.nbt.NBTTagCompound convert(LegacyType type, net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
        return convert(type.getDFUType(), cmp, sourceVer, targetVer);
    }

    public static net.minecraft.nbt.NBTTagCompound convert(TypeReference type, net.minecraft.nbt.NBTTagCompound cmp) {
        int i = cmp.contains("DataVersion", 99) ? cmp.getInt("DataVersion") : -1;
        return convert(type, cmp, i);
    }

    public static net.minecraft.nbt.NBTTagCompound convert(TypeReference type, net.minecraft.nbt.NBTTagCompound cmp, int sourceVer) {
        return convert(type, cmp, sourceVer, DATA_VERSION);
    }

    public static net.minecraft.nbt.NBTTagCompound convert(TypeReference type, net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
        if (sourceVer >= targetVer) {
            return cmp;
        }
        return (net.minecraft.nbt.NBTTagCompound) INSTANCE.fixer.update(type, new Dynamic<>(OPS_NBT, cmp), sourceVer, targetVer).getValue();
    }


    public interface DataInspector {
        net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer);
    }
    ...
    public interface DataConverter {
        ...
        net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp);
    }
    ...
    private static void convertCompound(LegacyType type, net.minecraft.nbt.NBTTagCompound cmp, String key, int sourceVer, int targetVer) {
        cmp.put(key, convert(type, cmp.getCompound(key), sourceVer, targetVer));
    }

    private static void convertItem(net.minecraft.nbt.NBTTagCompound nbttagcompound, String key, int sourceVer, int targetVer) {
        if (nbttagcompound.contains(key, 10)) {
            convertCompound(LegacyType.ITEM_INSTANCE, nbttagcompound, key, sourceVer, targetVer);
        }
    }

    private static void convertItems(net.minecraft.nbt.NBTTagCompound nbttagcompound, String key, int sourceVer, int targetVer) {
        if (nbttagcompound.contains(key, 9)) {
            net.minecraft.nbt.NBTTagList nbttaglist = nbttagcompound.getList(key, 10);
    ...
    private static class DataConverterEquipment implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            net.minecraft.nbt.NBTTagList nbttaglist = cmp.getList("Equipment", 10);
            net.minecraft.nbt.NBTTagList nbttaglist1;

            if (!nbttaglist.isEmpty() && !cmp.contains("HandItems", 10)) {
                nbttaglist1 = new net.minecraft.nbt.NBTTagList();
                nbttaglist1.add(nbttaglist.get(0));
                nbttaglist1.add(new net.minecraft.nbt.NBTTagCompound());
            ...
            if (cmp.contains("DropChances", 9)) {
                ...
                net.minecraft.nbt.NBTTagList nbttaglist2;
    ...
    private static class DataInspectorBlockEntity implements DataInspector {
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            if (!cmp.contains("tag", 10)) {
                return cmp;
            } else {
                net.minecraft.nbt.NBTTagCompound nbttagcompound1 = cmp.getCompound("tag");

                if (nbttagcompound1.contains("BlockEntityTag", 10)) {
                    net.minecraft.nbt.NBTTagCompound nbttagcompound2 = nbttagcompound1.getCompound("BlockEntityTag");
    ...
    private static class DataInspectorEntity implements DataInspector {
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            ...
            if (nbttagcompound1.contains("EntityTag", 10)) {
                net.minecraft.nbt.NBTTagCompound nbttagcompound2 = nbttagcompound1.getCompound("EntityTag");
                String s = cmp.getString("id");
                String s1;
    ...
    private abstract static class DataInspectorTagged implements DataInspector {
        ...
        abstract net.minecraft.nbt.NBTTagCompound inspectChecked(net.minecraft.nbt.NBTTagCompound nbttagcompound, int sourceVer, int targetVer);
    }
    ...
    private static class DataInspectorItemList extends DataInspectorTagged {
        ...
        net.minecraft.nbt.NBTTagCompound inspectChecked(net.minecraft.nbt.NBTTagCompound nbttagcompound, int sourceVer, int targetVer) {
                ...
                if (nbttagcompound1.contains("BlockEntityTag", 10)) {
                    ...
                    if (nbttagcompound2.contains("Base", 99)) {
                        ...
                        if (nbttagcompound1.contains("display", 10)) {
                            net.minecraft.nbt.NBTTagCompound nbttagcompound3 = nbttagcompound1.getCompound("display");

                            if (nbttagcompound3.contains("Lore", 9)) {
                                net.minecraft.nbt.NBTTagList nbttaglist = nbttagcompound3.getList("Lore", 8);
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            if (!"MobSpawner".equals(cmp.getString("id"))) {
                ...
                if (cmp.contains("EntityId", 8)) {
                    ...
                    net.minecraft.nbt.NBTTagCompound nbttagcompound1 = cmp.getCompound("SpawnData");
                ...
                if (cmp.contains("SpawnPotentials", 9)) {
                    net.minecraft.nbt.NBTTagList nbttaglist = cmp.getList("SpawnPotentials", 10);

                    for (int i = 0; i < nbttaglist.size(); ++i) {
                        net.minecraft.nbt.NBTTagCompound nbttagcompound2 = nbttaglist.getCompound(i);

                        if (nbttagcompound2.contains("Type", 8)) {
                            net.minecraft.nbt.NBTTagCompound nbttagcompound3 = nbttagcompound2.getCompound("Properties");
    ...
    private static class DataConverterSaddle implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            if ("EntityHorse".equals(cmp.getString("id")) && !cmp.contains("SaddleItem", 10) && cmp.getBoolean("Saddle")) {
                net.minecraft.nbt.NBTTagCompound nbttagcompound1 = new net.minecraft.nbt.NBTTagCompound();
    ...
    private static class DataConverterHanging implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            ...
            if ((flag || flag1) && !cmp.contains("Facing", 99)) {
                ...
                if (cmp.contains("EnumDirection", 99)) {
                    enumdirection = EnumDirection.from2DDataValue(cmp.getByte("EnumDirection"));
    ...
    private static class DataConverterRiding implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            while (cmp.contains("Riding", 10)) {
                net.minecraft.nbt.NBTTagCompound nbttagcompound1 = this.b(cmp);
        ...
        protected void convert(net.minecraft.nbt.NBTTagCompound nbttagcompound, net.minecraft.nbt.NBTTagCompound nbttagcompound1) {
        ...
        protected net.minecraft.nbt.NBTTagCompound b(net.minecraft.nbt.NBTTagCompound nbttagcompound) {
            net.minecraft.nbt.NBTTagCompound nbttagcompound1 = nbttagcompound.getCompound("Riding");
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            if ("minecraft:written_book".equals(cmp.getString("id"))) {
                ...
                if (nbttagcompound1.contains("pages", 9)) {
                    net.minecraft.nbt.NBTTagList nbttaglist = nbttagcompound1.getList("pages", 8);

                    for (int i = 0; i < nbttaglist.size(); ++i) {
                        String s = nbttaglist.getString(i);
                        IChatBaseComponent object = null;

                        if (!"null".equals(s) && !UtilColor.isNullOrEmpty(s)) {
                            if ((s.charAt(0) != 34 || s.charAt(s.length() - 1) != 34) && (s.charAt(0) != 123 || s.charAt(s.length() - 1) != 125)) {
                                object = IChatBaseComponent.literal(s);
                            } else {
                                try {
                                    object = ChatDeserializer.fromJson(DataConverterSignText.a, s, IChatBaseComponent.class, true);
                                    if (object == null) {
                                        object = IChatBaseComponent.literal("");
                                    }
                        ...
                        nbttaglist.set(i,
                                net.minecraft.nbt.NBTTagString.valueOf(IChatBaseComponent.ChatSerializer.toJson(object)));
                    }
    ...
    private static class DataConverterCookedFish implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            if (cmp.contains("id", 8) && DataConverterCookedFish.a.equals(new MinecraftKey(cmp.getString("id")))) {
    ...
    private static class DataConverterBedBlock implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            try {
                net.minecraft.nbt.NBTTagCompound nbttagcompound1 = cmp.getCompound("Level");
                int i = nbttagcompound1.getInt("xPos");
                int j = nbttagcompound1.getInt("zPos");
                net.minecraft.nbt.NBTTagList nbttaglist = nbttagcompound1.getList("TileEntities", 10);
                net.minecraft.nbt.NBTTagList nbttaglist1 = nbttagcompound1.getList("Sections", 10);

                for (int k = 0; k < nbttaglist1.size(); ++k) {
                    net.minecraft.nbt.NBTTagCompound nbttagcompound2 = nbttaglist1.getCompound(k);
                    ...
                    for (int l = 0; l < abyte.length; ++l) {
                        if (416 == (abyte[l] & 255) << 4) {
                            ...
                            int j1 = l >> 8 & 15;
                            int k1 = l >> 4 & 15;
                            net.minecraft.nbt.NBTTagCompound nbttagcompound3 = new net.minecraft.nbt.NBTTagCompound();
    ...
    private static class DataConverterBedItem implements DataConverter {
        ...
        public net.minecraft.nbt.NBTTagCompound convert(net.minecraft.nbt.NBTTagCompound cmp) {
            if ("minecraft:bed".equals(cmp.getString("id")) && cmp.getShort("Damage") == 0) {
                cmp.putShort("Damage", (short) EnumColor.RED.getId());
            }
    ...
    private static class DataConverterSignText implements DataConverter {
        ...
        public static final Gson a = new GsonBuilder().registerTypeAdapter(IChatBaseComponent.class, new JsonDeserializer() {
            IChatMutableComponent a(JsonElement jsonelement, Type type, JsonDeserializationContext jsondeserializationcontext) throws JsonParseException {
                if (jsonelement.isJsonPrimitive()) {
                    ...
                    throw new JsonParseException("Don't know how to turn " + jsonelement + " into a IChatBaseComponent");
                }
            }
        ...
        private void convert(net.minecraft.nbt.NBTTagCompound nbttagcompound, String s) {
            String s1 = nbttagcompound.getString(s);
            IChatBaseComponent object = null;

            if (!"null".equals(s1) && !UtilColor.isNullOrEmpty(s1)) {
                if ((s1.charAt(0) != 34 || s1.charAt(s1.length() - 1) != 34) && (s1.charAt(0) != 123 || s1.charAt(s1.length() - 1) != 125)) {
                    object = IChatBaseComponent.literal(s1);
                } else {
                    try {
                        object = ChatDeserializer.fromJson(DataConverterSignText.a, s1, IChatBaseComponent.class, true);
                        if (object == null) {
                            object = IChatBaseComponent.literal("");
                        }
                    } catch (JsonParseException jsonparseexception) {
            ...
            nbttagcompound.putString(s, IChatBaseComponent.ChatSerializer.toJson(object));
        }
    }
    ...
    private static class DataInspectorPlayerVehicle implements DataInspector {
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            if (cmp.contains("RootVehicle", 10)) {
                net.minecraft.nbt.NBTTagCompound nbttagcompound1 = cmp.getCompound("RootVehicle");
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            ...
            if (cmp.contains("entities", 9)) {
                ...
                for (j = 0; j < nbttaglist.size(); ++j) {
                    nbttagcompound1 = (net.minecraft.nbt.NBTTagCompound) nbttaglist.get(j);
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            if (cmp.contains("Level", 10)) {
                ...
                if (nbttagcompound1.contains("Entities", 9)) {
                    ...
                    for (j = 0; j < nbttaglist.size(); ++j) {
                        nbttaglist.set(j, convert(LegacyType.ENTITY, (net.minecraft.nbt.NBTTagCompound) nbttaglist.get(j), sourceVer, targetVer));
                    }
                }
                ...
                if (nbttagcompound1.contains("TileEntities", 9)) {
                    ...
                    for (j = 0; j < nbttaglist.size(); ++j) {
                        nbttaglist.set(j, convert(LegacyType.BLOCK_ENTITY, (net.minecraft.nbt.NBTTagCompound) nbttaglist.get(j), sourceVer, targetVer));
                    }
                }
            }
    ...
    private static class DataInspectorEntityPassengers implements DataInspector {
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            if (cmp.contains("Passengers", 9)) {
                net.minecraft.nbt.NBTTagList nbttaglist = cmp.getList("Passengers", 10);
    ...
    private static class DataInspectorVillagers implements DataInspector {
        ...
        @Override
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            if (entityVillager.equals(new MinecraftKey(cmp.getString("id"))) && cmp.contains("Offers", 10)) {
                net.minecraft.nbt.NBTTagCompound nbttagcompound1 = cmp.getCompound("Offers");

                if (nbttagcompound1.contains("Recipes", 9)) {
                    net.minecraft.nbt.NBTTagList nbttaglist = nbttagcompound1.getList("Recipes", 10);

                    for (int j = 0; j < nbttaglist.size(); ++j) {
                        net.minecraft.nbt.NBTTagCompound nbttagcompound2 = nbttaglist.getCompound(j);
        ...
        public net.minecraft.nbt.NBTTagCompound inspect(net.minecraft.nbt.NBTTagCompound cmp, int sourceVer, int targetVer) {
            if (tileEntityMobSpawner.equals(new MinecraftKey(cmp.getString("id")))) {
                if (cmp.contains("SpawnPotentials", 9)) {
                    ...
                    for (int j = 0; j < nbttaglist.size(); ++j) {
                        net.minecraft.nbt.NBTTagCompound nbttagcompound1 = nbttaglist.getCompound(j);
